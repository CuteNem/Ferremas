...
def inicioSesion(request):
    if request.method == 'POST':
        correoI = request.POST.get('username')
        claveI = request.POST.get('password')
        print(f'Intento de inicio de sesión con correo: {correoI} y clave: {claveI}')  # Solo para depuración, no dejar en producción
        logger.debug(f'Intento de inicio de sesión con correo: {correoI}')

        user = authenticate(username=correoI, password=claveI)
        if user is None:
            logger.error(f'Fallo de autenticación para el usuario: {correoI}')
            return redirect('mostrarLogin')

        # Suponiendo que obtener_usuario es una función definida para interactuar con una API
        ...
        if usuario_api:
            ...
            logger.error(f'Error al obtener detalles del usuario desde la API para: {correoI}')
